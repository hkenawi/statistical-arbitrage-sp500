"""
Trains one LSTMModel per sliding-window batch (23 total).
All training logic is encapsulated in LSTMModel.fit() — this script
is responsible only for data loading, model instantiation, and
orchestrating the batch loop.

Outputs per batch:
    src/train/checkpoints/lstm/batch_{i:02d}/latest.pt     — saved every epoch
    src/train/checkpoints/lstm/batch_{i:02d}/epoch_NNN.pt  — every N epochs
    src/train/checkpoints/lstm/batch_{i:02d}/final.pt      — end of training
...
_CKPT_DIR = Path(__file__).resolve().parent / "checkpoints" / "lstm"
...
def train_batch(batch_idx: int, cfg: dict) -> None:
    """Train LSTM on one sliding-window batch."""
    import pandas as pd
